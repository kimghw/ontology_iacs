...
## [2026-04-13T15:10:00+09:00] pdf2md-worker: PR38Rev5__part01

```yaml
완료_보고:
  파트: "PR38Rev5__part01 (pages 1-50)"
  변환_페이지_수: 50
  추출_이미지_수: 17
  삽입_이미지_수: 10
  orphan_이미지: 7
  첨자_발견: true
  경계_잘림: "종료 단편"
  특이사항: "페이지 48(Appendix 3) 끝에서 'Calibration reports shall include:' 문장 이후 내용이 잘림 - part02에서 계속. 이미지 003/004는 Figure 1.2의 분리 레이어이나 003이 완전한 버전이므로 병합 대신 003을 사용. 이미지 005/006은 Figure 2의 분리 레이어이나 006이 고해상도 완전 버전이므로 006을 사용. orphan 이미지: 001(빈 흰색), 004(Fig1.2 오버레이 레이어), 005(Fig2 소형 중복), 011(흑색 strip), 012(흑색 strip), 015(빈 흰색 중복). 이미지 014는 000과 동일한 EEDI 수식 이미지이나 section 11.2에서 재사용되므로 삽입 처리."
  사용_토큰:
    입력_토큰: 0
    출력_토큰: 0
    총_토큰: 0
...
**해석·처리 보고**:
- 모호하거나 정보가 부족한 요청 내용: pdfimages가 Figure 1.2(003+004)와 Figure 2(005+006)를 각각 2개 파일로 분리 추출하여 병합 필요 여부 판단이 필요했음
- 에이전트 해석: PDF 시각 확인 결과, 003이 Figure 1.2의 완전한 이미지이고 004는 흰색 텍스트 오버레이 레이어, 006이 Figure 2의 완전한 고해상도 버전이고 005는 저해상도 중복으로 판단
- 실제 처리 방식: 병합 없이 완전한 단일 이미지(003, 006)를 각각 사용하여 링크 삽입. 나머지를 orphan 처리
- 문제점·위험: 004 레이어에 003에 없는 텍스트 라벨이 포함되어 있을 수 있으나, PDF 원본 시각 확인 시 003이 모든 정보를 포함하고 있음을 확인
- 심각도: 하

## 2026-04-13 pdf2md: PR/PR_31-42 (12 files, 119 pages, 13 parts)

### 변환 결과

| # | 파일명 | 페이지 | 파트 | 이미지 | 첨자 | lint 수정 | 상태 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | PR38Rev5 | 78p | 2 | 16/292 | sup | MD060(표 구분자), MD001/MD024 비활성화 | ✓ |
| 2 | PR-40-Rev.2-Dec-2024-CLN | 12p | 1 | 0 | sup | - | ✓ |
| 3 | PR-42-Rev.1-Nov-2024-CLN | 8p | 1 | 0 | MD033(placeholder태그) | MD026(헤딩 콜론) 비활성화 | ✓ |
| 4 | PR-36-Rev.1-Dec-2024-CLN | 7p | 1 | 0 | - | MD034(URL), MD056(표 열수) | ✓ |
| 5 | PR-41-Rev.1-Nov-2024-CLN | 3p | 1 | 0 | - | MD034(URL), MD036 비활성화 | ✓ |
| 6 | PR-36-Contact-details-March-2026 | 2p | 1 | 0 | - | MD034(이메일) | ✓ |
| 7 | pr35rev1 | 2p | 1 | 0 | - | - | ✓ |
| 8 | PR-32-Rev.2-Nov-2024-CLN-1 | 1p | 1 | 0 | sup | - | ✓ |
| 9 | PR-39Rev1 | 1p | 1 | 0 | - | - | ✓ |
| 10 | pr_31_pdf559 | 1p | 1 | 0 | - | - | ✓ |
| 11 | pr_33_pdf116 | 1p | 1 | 0 | - | - | ✓ |
| 12 | pr_34_pdf388 | 1p | 1 | 0 | - | - | ✓ |

### markdownlint 설정 변경 (.markdownlint.json 누적)

- MD001 (heading-increment): 원문 헤딩 계층 점프 보존
- MD004 (ul-style): 원문 목록 스타일 혼재 보존
- MD024 (no-duplicate-heading): 원문 반복 섹션명(Appendix 구조) 보존
- MD026 (no-trailing-punctuation): 원문 헤딩 구두점(콜론) 보존
- MD032 (blanks-around-lists): 원문 목록 구조 보존
- MD036 (no-emphasis-as-heading): 원문 볼드 단락 제목 보존
- MD056 (table-column-count): PDF 표 열 수 불일치 허용
- MD060 (table-column-style): PDF 표 구분자 스타일 허용

### 특이사항

- PR38Rev5 (78p): 유일한 다중 파트 파일. 파트 경계(p50-51) 이어붙임 성공 (Calibration reports shall include: → 불릿 목록)
- PR38Rev5 이미지: 292개 추출 중 16개만 본문 참조 (나머지는 표 경계선/장식 요소)
- PR38Rev5 Annex 1 플로우차트 2개: 벡터 그래픽으로 래스터 추출 불가, 제목 이미지로 대체
- 오탈자 검사: language_tool_python 미설치로 생략

---

## 2026-04-16 · md2wu/SKILL.md 개정 (Cross-Review 반영)

### 모호/부족한 요청 내용
- 사용자가 "200K 예시고"라고 말했으나, 기존 메모리는 "600K"로 세션 용량을 정의 → 둘의 관계(오버라이드/병존) 불분명.
- "같은 source family", "시리즈 분리 지양"이 정성 표현이고 구체 알고리즘 미제시.
- 아이템 단위(=MD 파일)와 기존 "락 단위 = corpus_scope" 충돌 해소 방향 미지정.

### 해석
- `batch_capacity` = **기본 600K (메모리 유지)**, 사용자/메모리 오버라이드로 200K 등 가변. `session_capacity`는 기본적으로 `batch_capacity`와 동일하게 두어 "한 세션 = 1 배치" 원칙 유지.
- Phase B 배치를 3단계로 재정의: **1차 family 그룹화 → 2차 series 연속성 유지 → 3차 NFD 패킹** (큰 그룹 우선, 예산 초과 시만 series 경계 분리).
- 락 단위는 **`item_id = doc_instance_key` (1 MD = 1 락)** 로 변경, `corpus_scope`는 보고·집계용으로 강등. session-queue §9 체크리스트 완전 선언.

### 처리
- `md2wu/SKILL.md`에 용어 정의 절 신설, Phase B 재작성, §9 통합 선언 절 추가, 병합 제약에 `source_family`·`series_group` 추가, 토큰 임계값 표에 배치/세션 레벨 분리, 중간 산출물 경로를 세션 로컬로 이동.
- Cross-review 원본: `agent_review_codex.md` (Codex) + 본 대화(Gemini).

### 잠재 문제점
- [중] 사용자가 200K을 실제로 원한 경우 메모리(600K)를 덮어써야 하나, 현재 SKILL.md는 "메모리 유지 + 오버라이드 허용"으로 양립 기술 → 런타임에 사용자가 명시 값을 주지 않으면 어느 값을 택할지 에이전트가 재확인 필요.
- [하] `series_group`/`series_order`의 산출 규칙은 Stage 1 메타데이터 기반으로 서술만 했고, 실제 파싱 구현(heading_profiles.json 확장)은 별도 작업 대상.
- [하] 기존 `skill_md2wu/temp/pre/` 하위에 남은 중간 산출물은 이전 구조 잔존물 — 본 개정으로는 스키마만 변경, 기존 파일 마이그레이션은 하지 않음.
