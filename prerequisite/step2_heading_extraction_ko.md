...
처리 모드 분기는 §2.0의 핵심 분기 표를 참조한다. 에이전트 할당 규칙은 다음과 같다.

- **≤ {{split_threshold:64K}} 토큰**: 1 Document = 1 에이전트가 Pass 1–4와 토큰 측정까지 수행.
- **> {{split_threshold:64K}} 토큰**: documentSplit별 에이전트는 **Pass 1–3만** 수행. Coordinator가 결과와 문법 후보를 병합한 뒤 **문서 수준 Pass 4 에이전트**를 별도로 생성.
...
4. **정규식 사양 생성 및 전수 스캔** — LLM이 Pass 1 결과와 문법 초안에서 정규식 패턴(번호 체계, `Part`, `Chapter`, `Reg.`, `Art.` 등 키워드 접두사)을 도출하여 **JSON 배열**로 출력하고 러너에 전달한다. 러너 스크립트(`scripts/step2_regex_runner.py`)가 각 패턴을 컴파일하고(`re.error` 발생 시 `Warning` 기록, 무시하지 않음), 정규 입력에 대해 실행하여 **전수 스크립트 매치 셋**을 생성한다. 정규식 사양의 JSON 스키마와 다중 매치 해소 규칙은 §2.6 출력 스키마를 참조한다.
...
- **대상:** 사용자(프로젝트 운영자).
- **트리거:** Pass 3 반복 2회 후에도 `Error > 0`인 경우.
- **보고:** 최종 불일치 TSV(`discrepancy_final.tsv`)와 함께 잔존 Error 목록을 첨부하여 보고한다.
- **사용자 응답** (Step 3.3과 동일 체계):

| 응답 | 동작 |
|:---|:---|
| `proceed` | 잔존 Error를 인지하고 그대로 진행한다. 보고된 항목은 다운스트림에서 수동 검토가 필요하다. |
| `revise` | 정규식 사양 또는 grammar를 수동 보정한 뒤 Pass 2부터 재실행한다. |
| `abort` | 해당 Document 처리를 중단한다. 임시 사본을 `results/aborted/{doc_instance_key}/`로 격리 보존한다. |
...
- **권장 도구**: `tiktoken` (`cl100k_base` 인코딩).
...
| `Depth` | int | Yes | 트리 깊이: DocumentRoot = 0(가상 행), 첫 번째 제목 수준 = 1, 이후 1씩 증가. |
