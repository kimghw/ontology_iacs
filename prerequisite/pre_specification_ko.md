...
| 7b | 청크 계획 | `doc-{doc_instance_key}__heading__chunk_plan.json` | Step 3.1 | 청크 경계, 토큰 크기; 제목 구조 TSV와 별도. |
| 7c | 최종 불일치 로그 | `doc-{doc_instance_key}__heading__discrepancy_final.tsv` | Step 2 | 감사 추적을 위해 보존된 Warning/Info 항목 |
| 8 | 커버리지 보고서 | `doc-{doc_instance_key}__heading__coverage.json` | Step 2 Pass 4 | 줄 수준 분류 감사 (제목 vs 비제목). 항상 생성. |
...
| **3** | Step 3.3 | 이슈 게이트 및 매니페스트 확정 | [step3_workunit_packing.md](step3_workunit_packing.md) §3.3 | 청킹 계획 + WU 패킹 계획 + Document 목록 | 이슈 트리거 검사 → `corpus__pre__manifest.json` (`abort` 시 `results/aborted/{doc_instance_key}/`로 격리) |
...
| 5 | §3.3 이슈 트리거가 발생하지 않았거나, 발생한 경우 사용자 응답(`proceed`/`revise`/`abort`)이 처리되었다 | Step 3.3 |
| 6 | 모든 산출물이 `results/`에 직접 저장되었다 (`abort`된 항목은 `results/aborted/{doc_instance_key}/`로 격리) | Step 3.3 §산출물 저장 |
| 7 | `corpus__pre__manifest.json`이 생성되어 다운스트림에서 소비 가능하다 | Step 3.3 |

> `abort` 응답을 받은 Document는 manifest에 포함되지 않고 격리 폴더에 보존된다. 나머지 Document에 대해서는 다운스트림 진행이 가능하다.
...
| shared 파일 | 내용 |
|:---|:---|
| [`project_definitions.md`](../shared/project_definitions.md) | 용어(HD/TD/APP/CT), 식별자 체인(DocumentKey → Heading_ID), 토큰 측정 기준(Inclusive/Exclusive, 임계값), 수명 주기 상태 |
...
| 이슈 게이트 | §3.3 이슈 트리거 검사. 트리거 발생 시에만 사용자에게 보고하고 `proceed`/`revise`/`abort` 응답 처리 (`abort` 시 `results/aborted/`로 격리) |
...
```
planned → running → completed → validated → processed (정상 자동 완료)
                                           ↘ proceeded (이슈 게이트 proceed 응답)
                                           ↘ revised   (이슈 게이트 revise 응답 → 재실행)
                                           ↘ aborted   (이슈 게이트 abort 응답 → results/aborted/로 격리)
                  ↘ failed → retryable → running (재시도)
                             ↘ escalated (최대 재시도 초과)
```

> `merged`는 WU 수명 주기 상태가 아니다 — WU_Type (`standalone`, `split`, `merged`)이다.
...
| `documents[].status` | `processed`, `proceeded`, `revised`, `aborted` (이슈 게이트 응답 결과) |
| `work_units[]` | WU 항목 배열 |
...
| `work_units[].output_files[]` | `results/` 하위에 저장된 산출물 파일 경로 목록 |
...
> **참고:** `documents[].status`는 `corpus__pre__document_manifest.jsonl`의 `confirmed`에서 §3.3 이슈 게이트 처리 후 `corpus__pre__manifest.json`의 `processed`(정상 자동 완료) 또는 `proceeded`/`revised`/`aborted`(이슈 응답 결과)로 전환된다.
...
- *[step3_workunit_packing.md](step3_workunit_packing.md) — 청킹, WU 패킹, 이슈 게이트*
