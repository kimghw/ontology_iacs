...
![Annex 1 Typical Loading Sequence Summary Form: 선박 측면도 및 홀드 구획 다이어그램 상단에 배치되고, 하단에는 각 홀드·밸러스트 탱크별 화물/밸러스트 적재 단계, 잔류 화물, 최대 정수 중 굽힘 모멘트·전단력, 트림·흘수 등을 단계별로 기록하는 공란 양식 표가 포함된 Loading Sequence Summary Form 예시](assets/ur-s1arev6/part01-fig-annex1-merged.png)
...
### GUIDANCE FOR LOADING / UNLOADING SEQUENCES (2)

1. UR S1A.1 requires that Bulk Carriers, Ore Carriers and Combination Carriers of 150 m length and above, which are contracted for construction on or after 1st July 1998, are to be provided with an approved loading manual with typical loading sequences where the ship is loaded from commencement of cargo loading to reaching full deadweight capacity, for homogeneous conditions, relevant part loaded conditions and alternate conditions where applicable. The typical unloading sequences shall be developed paying due attention to the loading rate, the deballasting capacity and the applicable strength limitations.
