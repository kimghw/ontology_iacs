...
4.1.1 The pressures $p$, in kN/m<sup>2</sup> acting on air pipes, ventilator pipes and their closing devices may be calculated from:

$$p = 0.5 \rho V^2 C_d C_s C_p$$

where:

$\rho$ = density of sea water (1.025 t/m<sup>3</sup>)
$V$ = velocity of water over the fore deck
   = 13.5 m/sec for $d \leq 0.5\, d_1$

$$V = 13.5 \sqrt{2\left(1 - \frac{d}{d_1}\right)}\ \text{m/sec for } 0.5\, d_1 < d < d_1$$

$d$ = distance from summer load waterline to exposed deck
$d_1$ = *0.1L* or *22 m* whichever is the lesser
$C_d$ = shape coefficient
   = 0.5 for pipes, 1.3 for air pipe or ventilator heads in general, 0.8 for an air pipe or ventilator head of cylinderical form with its axis in the vertical direction.
...
- 200 kN/m<sup>2</sup> normal to the shaft axis and away from the forward perpendicular, over the projected area in this direction,

- 150 kN/m<sup>2</sup> parallel to the shaft axis and acting both inboard and outboard separately, over the multiple of $f$ times the projected area in this direction,
...
$A_i$ = cross sectional area of all bolts in group $i$ (cm<sup>2</sup>)
...
$$F_i = \left(F_{xi}^2 + F_{yi}^2\right)^{0.5}$$

where:

$\alpha$ = coefficient of friction (0.5)
$M$ = mass of windlass (tonnes)
$g$ = gravity acceleration (9.81 m/sec<sup>2</sup>)
...
5.1.8 Rotating type mushroom ventilator heads are to be unsuitable for application in the areas defined in 2.
...
| Nominal pipe diameter (mm) | Minimum fitted gross thickness, LL36(c) (mm) | Maximum projected area of head (cm<sup>2</sup>) | Height <sup>(1)</sup> of brackets (mm) |
|---|---|---|---|
...
| Nominal pipe diameter (mm) | Minimum fitted gross thickness, LL36(c) (mm) | Maximum projected area of head (cm<sup>2</sup>) | Height of brackets (mm) |
|---|---|---|---|
| 80A | 6.3 | - | 460 |
| 100A | 7.0 | - | 380 |
| 150A | 8.5 | - | 300 |
...
![Figure 1. Direction of Forces and Weight - windlass diagrams showing P_x, P_y forces, shaft height h, overall height H, weight W, and centre line of vessel vs centre line of windlass with angle B](assets/ur-s27rev6/part01-fig-000.png)

Figure 1. Direction of Forces and Weight

![Figure 2. Sign Convention - bolt group layout showing x_i, y_i coordinates relative to centroid of bolt groups with P_x, P_y force directions](assets/ur-s27rev6/part01-fig-001.png)
