...
![Diagram showing design load on fitting due to turn in towing line](https://i.imgur.com/example_towing_load_diagram.png)
*(Note: Diagram shows a fitting with a towing line passing through it at an angle, with "DESIGN LOAD ON LINE" vectors and a resultant "DESIGN LOAD ON FITTING" vector which is noted as "(NOT MORE THAN 2 TIMES DESIGN LOAD ON LINE)")*
...
![Diagram showing reinforcement beneath shipboard fittings](https://i.imgur.com/example_reinforcement_diagram.png)
*(Note: Diagram shows a plan and section view of reinforcing members like web frames and deck stiffeners beneath a fitting like a bollard or chock.)*
...
![Diagram showing acting point of towing force on bollard](https://i.imgur.com/example_bollard_acting_point.png)
*(Note: Diagram shows a bollard with height H, and the acting point of the load at H*4/5 from the base. It also shows an eye splice attachment on a double bollard.)*
...
3) TOW used for other towing operations is not to exceed 80% of the design load according to A2.1.3 (2) and 3) is to be used.
...
Shipboard fittings may be selected from an industry standard accepted by the Society and at least based on the ship design minimum breaking load according to IACS Recommendation No. 10 “Anchoring, Mooring and Towing Equipment” (see Notes in A2.2.3). Mooring bitts (double bollards) are to be chosen for the mooring line attached in figure-of-eight fashion so as to facilitate efficient distribution between different methods to attach the line, i.e. figure-of-eight or eye splice attachment.
...
![Diagram showing acting point of mooring force](https://i.imgur.com/example_mooring_acting_point.png)
*(Note: Diagram shows a bollard with acting point at 4/5 height (a) and at the fin location (b).)*
