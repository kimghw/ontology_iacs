...
| CU4        | 70-80   | 6,5-9,0  | 8,0-20,0 | max 6,0 | 2,0-5,0 | 1,5-3,0 | max 0,1 | max 0,05 |
...
$$\text{Zinc equivalent (\%)} = 100 - \frac{100 \cdot \%\text{Cu}}{100 + A}$$

In which A = %Sn + 5 x %Al - 0,5 x %Mn - 0,1 x %Fe - 2,3 x %Ni.
...
| Alloy type | Proof stress R<sub>p</sub> 0,2 [N/mm<sup>2</sup>] min. | Tensile strength R<sub>m</sub> [N/mm<sup>2</sup>] min. | Elongation A<sub>5</sub> [%] min. |
|------------|---------------------------------------------------------|---------------------------------------------------------|------------------------------------|
| CU1        | 175                                                     | 440                                                     | 20                                 |
| CU2        | 175                                                     | 440                                                     | 20                                 |
| CU3        | 245                                                     | 590                                                     | 16                                 |
...
![Fig.1 Test sample casting showing a tapered test coupon attached to a cylindrical section with dimensions H, B, L, T and D labelled](assets/UR-W24-Rev.5-Corr.1-Dec-2024-CLN-2/part01-fig-000.png)
...
![Fig.2 Definition of skew angle showing projected blade view with skew angle between leading edge line and locus of mid-chord line](assets/UR-W24-Rev.5-Corr.1-Dec-2024-CLN-2/part01-fig-001.png)
...
Zone A is in the area on the pressure side of the blade, from and including the fillet to 0,4R, and bounded on either side by lines at a distance 0,15 times the chord length C<sub>r</sub> from the leading edge and 0,2 times C<sub>r</sub> from the trailing edge, respectively (see Fig. 3). Where the hub radius (R<sub>b</sub>) exceeds 0,27R, the other boundary of zone A is to be increased to 1,5R<sub>b</sub>.
...
![Fig.3 Severity zones for integrally cast low skew propellers showing zones A, B and C on pressure and suction sides with boundaries at 0,4R, 0,7R, 0,15Cr and 0,2Cr](assets/UR-W24-Rev.5-Corr.1-Dec-2024-CLN-2/part01-fig-002.png)
...
![Fig.4 Severity zones in blades with skew angles greater than 25 degrees showing zones A and B on pressure and suction sides with boundaries at 0,4R, 0,7R, 0,9R, 0,5Cr, 0,3Cr and 0,15Cr](assets/UR-W24-Rev.5-Corr.1-Dec-2024-CLN-2/part01-fig-003.png)

Fig.4   Severity zones in blades with skew angles greater than 25°

![Fig.5 Severity zones for controllable pitch propeller boss showing cross-section with Zone A on inside and outside of bore d1 and Zone C on the remaining boss material, with d2 and 1,2 x d2 dimensions](assets/UR-W24-Rev.5-Corr.1-Dec-2024-CLN-2/part01-fig-004.png)

Fig.5   Severity zones for controllable pitch propeller boss

![Fig.6 Severity zones for controllable pitch and built-up propeller showing Section a-a through the blade, Section b-b through the flange fillet, and blade with Zone A around bore holes and fillet end](assets/UR-W24-Rev.5-Corr.1-Dec-2024-CLN-2/part01-fig-005.png)
...
![Fig.7 Shape of indications showing non-linear (l/W ≤ 3) and linear (l/W > 3) indication shapes, and alignment of non-linear and linear indications with gap distances d_i and total length l_T](assets/UR-W24-Rev.5-Corr.1-Dec-2024-CLN-2/part01-fig-006.png)

Fig.7   Shape of indications

### 10.2 Acceptance standard

The surface to be inspected is to be divided into reference areas of 100 cm<sup>2</sup>. Each reference area may be square or rectangular with the major dimension not exceeding 250mm.
...
Table 3   Allowable number and size of relevant indications in a reference area of 100 cm<sup>2</sup>, depending on severity zones<sup>1)</sup>

| Severity zones | Max. total number of indications | Type of indication | Max. number of each type<sup>1)2)</sup> | Max. acceptable value for "l" of indications [mm] |
|----------------|----------------------------------|--------------------|-------------------------------------------|----------------------------------------------------|
| **A**          | 7                                | Non-linear         | 5                                         | 4                                                  |
|                |                                  | Linear             | 2                                         | 3                                                  |
|                |                                  | Aligned            | 2                                         | 3                                                  |
...
Notes:
<sup>1)</sup> Singular non-linear indications less than 2 mm for zone A and less than 3 mm for the other zones are not considered relevant.

<sup>2)</sup> The total number of non-linear indications may be increased to the max. total number, or part thereof, represented by the absence of linear or aligned indications.
...
Welding of areas less than 5 cm<sup>2</sup> is to be avoided.

### 11.3 Repair of defects in zone A

In zone A, repair welding will generally not be allowed unless specially approved by the Classification Society.

Grinding may be carried out to an extent which maintains the blade thickness of the approved drawing.
...
Defects that are not deeper than dB (depth in zone B) = (t/40) mm or 2 mm (whichever is greatest) should be removed by grinding.
...
|------------|----------------------------------------------------------|-------------------------------|---------------------------------|------------------------------|----------------------------------|
| CU1        | Al-bronze<sup>1)</sup><br>Mn-bronze                      | 150                           | 300                             | 350-500                      | 500-800                          |
| CU2        | Al-bronze<br>Ni-Mn-bronze                                | 150                           | 300                             | 350-550                      | 500-800                          |
| CU3        | Al-bronze<br>Ni-Al-bronze<sup>2)</sup><br>Mn-Al-bronze   | 50                            | 250                             | 450-500                      | 700-900                          |
| CU4        | Al-Mn-bronze                                             | 100                           | 300                             | 450-600                      | 700-850                          |

Notes:
<sup>1)</sup> Ni-Al-bronze and Mn-Al-bronze are acceptable.

<sup>2)</sup> Stress relieving not required, if filler metal Ni-Al-bronze is used.
...
![Fig.A.1 Test piece for welding repair procedure showing two rectangular blocks joined along a central edge with dimensions a (width of each block), b (height) and t (material thickness), and joint preparation marker 1](assets/UR-W24-Rev.5-Corr.1-Dec-2024-CLN-2/part01-fig-008.png)
...
Fig.A.1 Test piece for welding repair procedure
...
![Fig.A.2 Test Specimen layout showing a ~300 mm square test piece partitioned top-to-bottom into Discard, Macro specimen, Tensile test specimen, Macro specimen, Tensile test specimen, Macro specimen, Discard, and a separate V-groove cross-section of thickness t](assets/UR-W24-Rev.5-Corr.1-Dec-2024-CLN-2/part01-fig-009-merged.png)

Fig.A.2     Test Specimen
...
| Alloy Type | Tensile Strength R<sub>m</sub> (N/mm<sup>2</sup>) min. |
|------------|---------------------------------------------------------|
| CU1        | 370                                                     |
| CU2        | 410                                                     |
| CU3        | 500                                                     |
