I will now check the contents of the `UR/UR_E` directory to locate the specified PDF file.
I will now read the PDF file `UR-E21Rev2-CLN.pdf` and convert its content into a well-formatted Markdown.
...
![Double Conversion topology diagram](Double_Conversion_Topology.png)
*(Diagram showing: AC input -> Rectifier -> DC link (with Energy storage device connected) -> Inverter -> AC output)*
...
![Line interactive topology diagram](Line_Interactive_Topology.png)
*(Diagram showing: AC input -> Power interface -> AC output; with a Bidirectional converter connected between the interface and the Energy storage device)*
...
![Standby topology diagram](Standby_Topology.png)
*(Diagram showing: AC input -> UPS switch -> AC output; with Battery charger and Inverter path leading from AC input to the UPS switch via Energy storage device)*
