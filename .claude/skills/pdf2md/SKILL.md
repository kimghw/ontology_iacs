...
본 스킬은 PDF 파일을 원문 손실 없이 구조화된 마크다운으로 변환하는 절차를 정의한다. 50페이지 단위로 서브에이전트(opus 모델)를 병렬 할당하고, 각 에이전트는 담당 페이지 추출물을 Read 도구로 직접 읽어 변환하며, 이미지는 별도 파일로 추출하여 링크로 참조한다. 대량 처리 시 **배치 단위 실행 → `/compact` → 재호출** 사이클로 컨텍스트를 관리한다.
...
- **배치(Batch)**: 한 번의 실행 사이클에서 처리하는 서브에이전트 작업 묶음. 최대 20개 서브에이전트로 구성되며, **한 소스 파일의 모든 파트는 반드시 같은 배치에 포함**된다. 배치 완료 후 `/compact` → 스킬 재호출로 다음 배치를 처리한다.
- **batch_state.json**: 배치 간 상태를 유지하는 진행 파일. `/compact` 후에도 이 파일을 읽어 작업을 이어간다.
...
- **작업 디렉토리 레이아웃**:
  ...
  ```text
  <workroot>/
  ├── batch_state.json ← 배치 간 진행 상태 (compact 후 재개 시 읽음)
  ├── queue/
...
- **batch_state.json 구조**:
  ...
  ```json
  {
    ...
    "current_batch": 1,
    "skipped_files": ["already_done.pdf"],
    "completed_files": ["file_a.pdf", "file_b.pdf"],
    "failed_files": []
  }
  ```
...
- **배치 단위 실행**: 전체 작업을 **배치**(최대 20 서브에이전트)로 분할한다. **한 소스 파일의 모든 파트는 반드시 같은 배치에 포함**되어야 한다. 파일을 순서대로 배치에 채우되, 다음 파일을 추가하면 20을 초과할 경우 새 배치를 시작한다. 단일 파일이 20파트를 초과하면 해당 파일이 단독 배치를 형성한다.
- **배치 간 `/compact` → 재호출**: 한 배치의 모든 서브에이전트 완료 + 병합·검증이 끝나면 `batch_state.json`을 갱신하고, 남은 배치가 있으면 `/compact`를 실행한 뒤 pdf2md 스킬을 남은 작업에 대해 재호출한다. 이로써 컨텍스트 누적을 방지한다.
- **병렬 실행**: 배치 내에서는 단일 메시지 내 다중 Agent 병렬 호출로 모든 서브에이전트를 동시 기동한다.
...
0. **재개 판정**
   - `<workroot>/batch_state.json`이 존재하는지 확인한다.
   - **존재하면(재개 모드)**: 파일을 읽어 `current_batch`를 확인하고, 해당 배치의 파일들에 대해 절차 5(워커 루프)부터 시작한다. 절차 1~4b는 건너뛴다.
   - **존재하지 않으면(최초 실행)**: 절차 1부터 순차 진행한다.
...
4b. **배치 구성**
   - 변환 대상 파일 목록을 순서대로 순회하며 배치를 구성한다.
   - **규칙**:
     - 배치 1개의 서브에이전트 수 상한은 **20**.
     - **한 소스 파일의 모든 파트는 반드시 같은 배치에 포함**된다(파일 단위 원자성).
     - 파일을 순서대로 현재 배치에 추가하되, 다음 파일의 파트 수를 추가하면 20을 초과할 경우 새 배치를 시작한다.
     - 단일 파일의 파트 수가 20을 초과하면 해당 파일이 단독 배치를 형성한다(상한 초과 허용).
   - **예시**: 파일 A(3파트), B(8파트), C(6파트), D(5파트), E(12파트)
     - 배치 0: A + B + C = 17파트 (D를 추가하면 22 → 초과)
     - 배치 1: D + E = 17파트
   - `batch_state.json`을 생성하여 `<workroot>/`에 저장한다(2절 구조 참조). `current_batch`를 `0`으로 설정.

5. **워커 루프** (배치 단위 실행)
   - `batch_state.json`에서 `current_batch`에 해당하는 배치의 파일 목록을 확인한다.
   - 해당 파일들의 작업 파일만 `pending/`에서 `working/`으로 원자 이동(`mv`).
   - 이동한 **모든** 작업을 Agent 도구로 opus 서브에이전트에 위임. **단일 메시지 내 다중 Agent 호출**로 배치 내 전체를 병렬 기동한다.
...
8. **배치 전환 판정**
   - 현재 배치의 모든 파일에 대해 병합·검증(절차 6~7)이 완료되면 `batch_state.json`을 갱신한다:
     - 현재 배치의 `status`를 `"done"`으로 변경.
     - 완료된 파일들을 `completed_files`에 추가.
     - 실패한 파일이 있으면 `failed_files`에 추가.
   - **남은 배치가 있으면**:
     1. `current_batch`를 다음 배치 인덱스로 갱신하여 `batch_state.json`에 저장한다.
     2. 사용자에게 현재 배치 완료 현황과 남은 배치 수를 보고한다.
     3. `/compact`를 실행한다.
     4. `/compact` 직후, 사용자에게 다음 메시지를 출력한다: `"/pdf2md <workroot> 재개"` 형태로 pdf2md 스킬을 재호출하도록 안내한다. 단, **오케스트레이터가 직접 재호출**한다 — `batch_state.json`의 `workroot` 경로를 입력으로 하여 pdf2md 스킬을 호출하면 절차 0(재개 판정)에서 자동으로 다음 배치를 이어간다.
   - **남은 배치가 없으면**: 절차 9(최종 정리)로 진행한다.
...
9. **최종 경로 검증 및 정리**
   ...
   - **정리**: 위 검증이 모두 통과하면 `<workroot>/queue/` 전체를 삭제하고(`rm -rf <workroot>/queue`), `<workroot>/assets/`와 `batch_state.json`도 삭제한다. 결과적으로 `<workroot>/`는 비거나 완전히 제거되어도 무방하다.
...
- 배치 내 모든 서브에이전트는 단일 메시지 내 병렬 호출로 기동한다.
- 배치 완료 후 `batch_state.json`을 갱신하고, 남은 배치가 있으면 `/compact` → pdf2md 재호출로 다음 배치를 이어간다.
- 재개 시 `batch_state.json`을 읽어 `current_batch`부터 절차 5를 재개한다.
...
- [ ] `batch_state.json` 존재 여부 확인 → 재개 모드 판정
...
- [ ] 배치 구성: 파일 단위 원자성 유지, 배치당 ≤20 서브에이전트, `batch_state.json` 생성

**배치 실행 (절차 5~7, 배치마다 반복)**

- [ ] 4.1/4.2만 prompt에 포함하여 슬롯 치환, 말미에 정적 지시문 파일 경로 안내
- [ ] 배치 내 모든 opus 서브에이전트를 단일 메시지 내 병렬 호출로 기동
...
- [ ] `batch_state.json` 갱신: 현재 배치 `status: "done"`, `completed_files` 추가
- [ ] 남은 배치 있으면: `current_batch` 갱신 → `/compact` → pdf2md 재호출
- [ ] 남은 배치 없으면: 절차 9(최종 정리)로 진행

**최종 정리 (절차 9, 모든 배치 완료 후)**

- [ ] 경로 검증: `<원본폴더>_md/<input>.md` 존재, 이미지 개수 일치, 모든 이미지 링크 해소
- [ ] 사용자 보고 + `agent_report.md` append 완료
- [ ] 검증 통과 후 `<workroot>/queue/`, `<workroot>/assets/`, `batch_state.json` 삭제 — 검증 실패 시 보존
