...
본 스킬은 **오케스트레이터**, **서브에이전트**, **감시자(Supervisor)** 세 주체의 책임을 명확히 분리한다.

- **3절 오케스트레이터 작업 범위**: 스킬 실행자(사용자와 대화하는 주체)가 수행하는 작업. 루트 준비, 분해, 큐 관리, 병합, 검증, 정리.
- **3.7절 감시자(Supervisor)**: 오케스트레이터가 스킬 규칙을 준수하는지 검증하는 독립 에이전트. 매 라운드 서브에이전트 기동 직후 병렬로 호출되어, 오케스트레이터가 조립한 프롬프트를 스킬 원문과 대조·검증한다.
...
- **감시자(Supervisor)**: 오케스트레이터가 스킬 규칙(특히 3.4 프롬프트 조립 규칙)을 준수하는지 독립적으로 검증하는 에이전트. 오케스트레이터가 서브에이전트에 전달한 프롬프트를 SKILL.md 4절 원문과 대조하여 축약·생략·재작성 여부를 판정하고, 위반 시 오케스트레이터에게 차단 보고한다.
...
오케스트레이터가 서브에이전트에게 작업을 할당할 때 스킬 지침(3절·4절)을 준수하는지 중간 검증하는 에이전트.

#### 동작

1. **매 라운드 서브에이전트 기동 직후**, 감시자 에이전트 1개를 백그라운드로 호출한다.
2. 감시자는 해당 라운드의 서브에이전트 프롬프트 중 **1개 이상을 샘플링**하여 다음을 확인한다:
   - 4절 템플릿이 축약·생략·재작성 없이 전달되었는가?
   - 조건 플래그(`part_index`, `is_first_part` 등)가 task.json과 일치하는가?
   - 경로(`part_source`, 출력 디렉토리)가 실제 존재하는가?
3. 위반 발견 시 **BLOCK**을 반환하고, 오케스트레이터는 해당 라운드 산출물을 채택하지 않고 위반을 수정한 뒤 재기동한다.
4. 위반 내용은 사용자에게 보고하고 `agent_report.md`에 append한다.
