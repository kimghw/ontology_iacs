...
- **결과 큐(result_queue/)**: 서브에이전트가 완료되면 오케스트레이터가 해당 작업을 `working/` → `result_queue/`로 이동시키는 중간 큐. 오케스트레이터는 `result_queue/`를 **선입선출(FIFO)** 순서로 소비하며, 각 항목에 대해 메타데이터 파싱 → `done/` 이동 → 병합 가능 판정 → 병합·검증(절차 6~7)을 순차 수행한다. 이 큐가 존재함으로써 서브에이전트 완료(생산)와 오케스트레이터 후처리(소비)가 분리된다.
...
- **작업 디렉토리 레이아웃**:
  ...
  │   ├── result_queue/ ← 서브에이전트 완료 → 오케스트레이터 소비 대기 (FIFO)
  │   └── done/         ← 오케스트레이터가 후처리 완료한 작업 (병합 입력)
...
- **큐 기반 작업 관리 (생산자-소비자)**: 작업 단위(= 한 구간)는 `pending → working → result_queue → done` 디렉토리를 순차 이동. `mv`로 원자 이동하여 점유 경합을 막는다. 서브에이전트(생산자)가 완료되면 `working/` → `result_queue/`로 이동하고, 오케스트레이터(소비자)가 `result_queue/`에서 꺼내 후처리 후 `done/`으로 이동한다.
- **비동기 병렬 실행**: 서브에이전트는 `run_in_background: true`로 기동하여 오케스트레이터가 블로킹되지 않게 한다. 동시 기동 수는 최대 20개(절대 상한).
- **즉시 소비**: 서브에이전트 완료 알림이 도착하면 오케스트레이터는 해당 결과를 `result_queue/`에 넣고, 큐에서 순서대로 꺼내 후처리(메타데이터 파싱, done 이동, 병합 가능 판정)를 수행한다. 한 소스 파일의 모든 파트가 `done/`에 모이면 병합(절차 6)·검증(절차 7)을 즉시 수행한다.
...
5. **워커 루프** (생산자-소비자 패턴)
   ...
   **5a. 생산 — 서브에이전트 기동**
   ...
   - 이동한 **모든** 작업을 Agent 도구로 opus 서브에이전트에 위임. **`run_in_background: true`로 기동**한다. 단일 메시지 내 다중 Agent 호출로 동시 기동한다.
   ...
   **5b. 소비 — 결과 큐 처리 (완료 알림 도착 시마다)**
   - 서브에이전트 완료 알림이 도착하면 오케스트레이터는 즉시:
     1. 해당 작업 파일과 생성된 `partNN.md`를 `working/` → `result_queue/`로 이동.
     2. `result_queue/`에서 항목을 **순서대로** 꺼내 후처리:
        a. 완료 보고를 파싱하여 파일별 메타데이터에 누적(특히 `첨자 발견` 플래그, `추출 이미지 수` 합계).
        b. 후처리 완료된 항목을 `result_queue/` → `done/`으로 이동.
        c. 해당 입력 파일의 모든 파트가 `done/`에 모였으면 절차 6(병합)·절차 7(검증)을 **즉시** 수행한다.
     3. 실패한 작업은 `working/` → `pending/`으로 되돌리고 재시도 카운트를 증가. 임계(기본 2회) 초과 시 `failed/`로 분리 후 보고.

   **5c. 연속 파이프라인 — 추가 기동**
   - `result_queue/` 소비 후 `pending/`에 잔여 파트가 있고 현재 `working/` 수가 20 미만이면, 추가 파트를 `working/`으로 이동하여 새 서브에이전트를 `run_in_background: true`로 기동한다.
   - 이로써 라운드 경계 없이 **연속 파이프라인**으로 동작한다.

   **5d. 종료 조건**
   - `pending/`, `working/`, `result_queue/`가 모두 비고, 모든 파일의 병합·검증이 완료되면 절차 8(최종 정리)로 진행한다.
...
8. **최종 경로 검증 및 정리**
   - **경로 검증**: 전체 변환 대상 PDF에 대해 다음을 모두 확인한다.
     ...
     - `pending/`·`working/`·`result_queue/`가 비어 있고, 모든 구간이 `done/`에 존재(실패 항목은 `failed/`로 분리되어 보고됨).
...
- 큐 이동(`pending → working → result_queue → done`)은 `mv`로 원자 처리한다.
- 서브에이전트는 `run_in_background: true`로 기동한다. 완료 알림 도착 시 `result_queue/`로 이동 후 즉시 소비한다.
- `result_queue/`를 순서대로 소비하며, 소비 후 `pending/`에 잔여 파트가 있으면 추가 서브에이전트를 기동한다(연속 파이프라인).
...
- [ ] `<workroot>/queue/{pdf_parts,pending,working,result_queue,done}` 및 `assets/`, `out/` 생성
...
- [ ] 모든 opus 서브에이전트를 `run_in_background: true`로 기동

**소비 — 결과 큐 처리 (절차 5b, 완료 알림마다 반복)**

- [ ] 완료된 작업을 `working/` → `result_queue/`로 이동
- [ ] `result_queue/`에서 순서대로 꺼내 완료 보고 파싱, `첨자 발견` 플래그·`추출 이미지 수` 누적
- [ ] 후처리 완료 항목을 `result_queue/` → `done/`으로 이동
...
- [ ] 소비 후 `pending/`에 잔여 파트 + `working/` < 20이면 추가 서브에이전트 기동
- [ ] `pending/`, `working/`, `result_queue/`가 모두 비면 → 절차 8(최종 정리)로 진행
