...
- **라운드(Round)**: 한 번의 실행 사이클에서 기동하는 서브에이전트 묶음. **라운드당 서브에이전트 수는 최대 20개(절대 상한)**. 전체 파트가 20을 초과하면 여러 라운드에 걸쳐 순차 처리한다. 한 라운드 완료 후 `pending/`에 잔여 파트가 있으면 즉시 다음 라운드를 시작한다.
...
- **라운드 단위 실행**: `pending/`에서 최대 20개 파트를 꺼내 한 라운드로 처리한다. 라운드 완료 후 `pending/`에 잔여 파트가 남아 있으면 즉시 다음 라운드를 시작한다. 모든 파트가 `done/`에 모일 때까지 반복한다.
- **병렬 실행**: 라운드 내에서는 단일 메시지 내 다중 Agent 병렬 호출로 모든 서브에이전트를 동시 기동한다.
- **라운드 간 병합**: 한 소스 파일의 모든 파트가 `done/`에 모이면 해당 파일의 병합·검증을 즉시 수행한다. 일부 파트만 완료된 파일은 나머지 파트가 `done/`에 도착할 때까지 병합을 보류한다.
...
5. **워커 루프** (라운드 반복)
   ...
   - 이동한 **모든** 작업을 Agent 도구로 opus 서브에이전트에 위임. **단일 메시지 내 다중 Agent 호출**로 라운드 내 전체를 병렬 기동한다.
   ...
   - **라운드 종료 후 병합 가능 파일 처리**: 한 입력 파일의 모든 파트가 `done/`에 모였으면 절차 6(병합)을 즉시 수행한다. 일부 파트만 완료된 파일은 보류.
   - **다음 라운드 판정**: `pending/`에 잔여 파트가 남아 있으면 절차 5 처음으로 돌아가 다음 라운드를 시작한다. `pending/`이 비었으면 절차 7(검증)로 진행한다.
...
- 라운드 내 모든 서브에이전트는 단일 메시지 내 병렬 호출로 기동한다.
- 라운드 완료 후 `pending/`에 잔여 파트가 있으면 즉시 다음 라운드를 시작한다.
...
- [ ] `pending/`에 잔여 파트 있으면 → 절차 5 처음으로 돌아가 다음 라운드
- [ ] 모든 파트가 `done/`에 모인 파일은 라운드 종료 시 즉시 병합(절차 6)
- [ ] `pending/`이 비었으면 → 절차 7(검증)로 진행
