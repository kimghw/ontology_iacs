...
### B4 파일·경로 등록 검증 (files_registry)

문서에 등장하는 파일명·경로가 `shared/files_registry.yaml` (SSOT)에 등록되어 있는지 확인한다.

1. **실행**: `python3 scripts/validate_files.py`
2. **결과 분석**: 미등록 항목을 `[artifact] / [path] / [doc]` 카테고리로 분류하여 검토한다.
   - 오타·패턴 불일치 → 문서 수정
   - 의도된 신규 파일 → `shared/files_registry.yaml` 에 추가
3. **스크립트가 못 잡는 부분은 에이전트가 판단**: 산문 속 약식 언급, placeholder 네이밍 차이(`{doc_instance_key}` vs `{docInstanceKey}`), 파일 실제 존재 여부 등은 에이전트가 문서 전체를 읽고 직접 확인·수정한다.
4. **판정**: 스크립트 exit 0 + 에이전트 검토 이상 없음이면 PASS.
